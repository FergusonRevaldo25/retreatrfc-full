...
function resultBadge(f: Fixture) {
  ...
  if (f.home_score > f.away_score) return { label: "WON", color: "bg-green-700 text-white" };
  if (f.home_score < f.away_score) return { label: "LOST", color: "bg-red-800 text-white" };
    ...
    <ul className="space-y-4">
      {fixtures.map((f) => {
        ...
        return (
          <li
            key={f.id}
            className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 border border-purple-800 rounded-md px-6 py-4 bg-neutral-900"
          >
            <div>
